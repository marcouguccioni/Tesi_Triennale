...
//IL PROGRAMMA SIMULA LA MASTER EQUATION PER NETWORK ERDOS G(50,0.2)
...
{  
       ...
       double t,m=0;
       char c;
       for(int i=0; i<vertex; i++) p0[i]=1./((double)vertex);
       ...
       cin>>prob;
        ...
        { 
         ...
         { 
           ...
           if(theta<prob || theta==prob) {adj[i][j]=adj[j][i]=true;}
       ...
       cout<<endl<<"Questo programma risolve la Master Equation per un Random Walk di N particelle non interagenti su un Network a Capacità di Trasporto infinita (ITC) di tipo G(n,p) [modello di Erdos-Renyi]. Si rappresenta graficamente l'andamento temporale della frazione di walkers sui nodi del network, con popolazione iniziale distribuita equamente sui nodi." <<endl;
